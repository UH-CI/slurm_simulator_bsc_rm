...
int main(int argc, char* argv[])
{
    int i;
    int idx=0, errs=0;
    int nrecs=2583275; // Number of records has to be entered here each time. This should be done differently.
    job_trace_t* job_trace,* job_trace_head,* job_arr,* job_ptr;
    char const* const fileName = argv[1]; /* should check that argc > 1 */
    FILE* file = fopen(fileName, "r"); /* should check the result */
    char line[256];
    char *p;
    const char* partitions[] = {"shared","gpu","srun","sandbox","exclusive","shared-long"};
	...
	if (nrecs%idx==0) {printf("%d / %d  \n",idx,nrecs);} 
        ...
        i=0;
        while(p!=NULL){
            if(i==0) { job_arr[idx].job_id = atoi(p); }   
            if(i==1) {  job_arr[idx].submit = 100 + atoi(p); }  // why submit cannot start from 0? 
            if(i==3) {  job_arr[idx].duration = atoi(p); }   
            //if(i==7) { printf("%s", p); job_arr[idx].tasks = ceil((float) (atoi(p)/CPUS_PER_NODE)); }
            if(i==7) {  job_arr[idx].cpus_per_task = ceil((float) atoi(p)); }
            if(i==8) {  job_arr[idx].wclimit = atoi(p) ; }   
            //if(i==11) { printf("%s", p); strcpy(job_arr[idx].username, p); }   
            //if(i==12) { printf("%s", p); strcpy(job_arr[idx].account, p); }   
            if(i==15) {  strcpy(job_arr[idx].partition, partitions[atoi(p)-1]); }   
            //printf(" %s\n", p);
            p = strtok(NULL," ");
            i++; 
        }
